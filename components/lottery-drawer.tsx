...
export function LotteryDrawer() {
   ...
   const handleFileUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
      ...
      const employeeList: Employee[] = lines
         .map((line, index) => ({
      ...
      <div className='w-full max-w-4xl mx-auto space-y-6'>
         ...
         <div className='flex justify-center gap-3'>
            ...
            <Button
               ...
               size='lg'
               className={`font-bold text-lg px-8 h-12 ${
                  mode === 'employees' ? 'bg-gradient-to-r from-blue-600 to-cyan-600 hover:from-blue-700 hover:to-cyan-700 text-white' : ''
               }`}
            >
               ...
               <div className='flex justify-center mb-4'>
                  <div
                     className={`p-4 rounded-2xl shadow-lg ${
                        mode === 'numbers' ? 'bg-gradient-to-br from-purple-500 to-pink-500' : 'bg-gradient-to-br from-blue-500 to-cyan-500'
                     }`}
                  >
                     {mode === 'numbers' ? <Sparkles className='w-12 h-12 text-white' /> : <Users className='w-12 h-12 text-white' />}
                  </div>
               </div>
               <CardTitle
                  className={`text-4xl font-bold bg-clip-text text-transparent ${
                     mode === 'numbers' ? 'bg-gradient-to-r from-purple-600 to-pink-600' : 'bg-gradient-to-r from-blue-600 to-cyan-600'
                  }`}
               >
                  {mode === 'numbers' ? 'قرعة الأرقام' : 'قرعة الموظفين'}
               </CardTitle>
               <CardDescription className='text-lg mt-2'>
                  {mode === 'numbers' ? 'اختر أرقامك العشوائية بطريقة عصرية وسهلة' : 'ارفع ملف CSV للموظفين واختر الفائزين'}
               </CardDescription>
            </CardHeader>
                  ...
                  <>
                     <div className='grid gap-6 md:grid-cols-2'>
                        <div className='space-y-2'>
                           ...
                           <Input
                              ...
                              disabled={isDrawing}
                           />
                           <p className='text-sm text-muted-foreground'>أدخل العدد الكلي للأرقام المتاحة</p>
                        </div>
                        ...
                        <div className='space-y-2'>
                           ...
                           <Input
                              ...
                              disabled={isDrawing}
                           />
                           <p className='text-sm text-muted-foreground'>أدخل عدد الأرقام التي تريد اختيارها</p>
                        </div>
                     </div>
                     ...
                     <div className='flex gap-3 justify-center'>
                        ...
                        {drawnNumbers.length > 0 && !isDrawing && (
                           <Button onClick={resetNumbers} variant='outline' size='lg' className='font-bold text-lg px-8 h-12'>
                              إعادة تعيين
                           </Button>
                        )}
                     </div>
                  ...
                  <>
                     <div className='space-y-6'>
                        <div className='space-y-2'>
                           ...
                           <p className='text-sm text-muted-foreground'>ارفع ملف CSV يحتوي على أسماء الموظفين (اسم واحد في كل سطر)</p>
                        </div>
                           ...
                           <>
                              ...
                              <div className='space-y-2'>
                                 ...
                                 <Input
                                    ...
                                    disabled={isDrawing}
                                 />
                                 <p className='text-sm text-muted-foreground'>أدخل عدد الموظفين الفائزين (من 1 إلى {employees.length})</p>
                              </div>
                           </>
                        )}
                     </div>
                     ...
                     <div className='flex gap-3 justify-center'>
                        ...
                        {employees.length > 0 && !isDrawing && (
                           <Button onClick={resetEmployees} variant='outline' size='lg' className='font-bold text-lg px-8 h-12'>
                              إعادة تعيين
                           </Button>
                        )}
                     </div>
            ...
            <Card className='border-2 shadow-xl animate-in fade-in slide-in-from-bottom-4 duration-500'>
               <CardHeader>
                  <div className='flex justify-between items-center flex-wrap gap-4'>
                     ...
                     <Button onClick={exportNumbersToCSV} variant='outline' size='lg' className='gap-2 font-bold'>
            ...
            <Card className='border-2 shadow-xl animate-in fade-in slide-in-from-bottom-4 duration-500'>
               <CardHeader>
                  <div className='flex justify-between items-center flex-wrap gap-4'>
                     ...
                     <Button onClick={exportEmployeesToCSV} variant='outline' size='lg' className='gap-2 font-bold'>
                        <Download className='h-5 w-5' />
                        تصدير CSV
                     </Button>
                  </div>
                              ...
                              <CardContent className='p-4'>
                                 <div className='flex items-center gap-3'>
                                    ...
                                    <div className='flex-1 min-w-0'>
                                       <p className='text-lg font-bold text-gray-900 dark:text-gray-100 truncate'>{employee.name}</p>
                                    </div>
                                 </div>
                              </CardContent>
                           </Card>
